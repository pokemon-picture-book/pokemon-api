...
export class setupSeedData1626941083645 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<any> {
        ...
        for (const pokemonGameImage of pokemonGameImages as []) {
            const { pokemonId, path, gameVersionGroupId, isMain } = pokemonGameImage;
            const isMainNum = isMain ? 1 : 0;
            await queryRunner.query(`INSERT INTO pokemon_game_images (pokemon_id, path, game_version_group_id, is_main) VALUES (${pokemonId}, '${path}', ${gameVersionGroupId}, ${isMainNum});`, undefined);
        }
