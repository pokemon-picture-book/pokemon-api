...
describe('Integration test for pokemon', () => {
    ...
    describe('Integration test for search all pokemon', () => {
        test('正常: クエリパラメータなしでリクエストした際に一番古い地域・ゲームバージョンで英語のデータが取得できているか', (done) => {
            request(server)
                ...
                .expect(200)
                .then((response) => {
                    const pokemons: SearchAllPokemonResponse = response.body;
    ...
    describe('Integration test for search one pokemon', () => {
        ...
        test('正常：存在しないポケモン ID でリクエストした場合、404 となるか', (done) => {
            request(server)
                .get(`${ROUTING.API}${ROUTING.POKEMON}/9999`)
                .expect('Content-Type', /json/)
                .expect(404, done);
        });

        test('正常：ID に Number.MAX_SAFE_INTEGER よりも大きい数値を指定した場合、400 となるか', (done) => {
            request(server)
                .get(
                    `${ROUTING.API}${ROUTING.POKEMON}/${
        ...
        test('正常：クエリパラメータなしでリクエストした場合、英語のデータが取得できるか', (done) => {
            request(server)
                .get(`${ROUTING.API}${ROUTING.POKEMON}/1`)
                .expect('Content-Type', /json/)
                .expect(200)
        ...
        test('異常：path parameter に文字列を入力した際に 400 となるか', (done) => {
            request(server)
                .get(`${ROUTING.API}${ROUTING.POKEMON}/hoge`)
                .expect('Content-Type', /json/)
                .expect(400, done);
        });
